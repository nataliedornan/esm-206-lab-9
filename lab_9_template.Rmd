...
# fct_relevel() function in forecats
...
full_aic; final_aic #runs code together so you can see in document not console

# Answer: which would you pick? 
# pick model homes_lm1 as the AIC value is lowest
...
stargazer(homes_lm1, homes_lm3, type = "html") # creates beeeeaauuuuutiful summary table for model outputs
...
## this model orders your column data in the order you specify
...
price_predict <- predict(homes_lm3, newdata = df_new, se.fit = T, interval = "confidence")

##class lets you see if your data are numeric or character, note that without additional data it will remain in vector form
...
predict_graph <- ggplot(predict_df, aes(x = SqFt, y = fit.fit)) +
  ...
  geom_point(data = homes_sub, aes(x = SqFt, y = Price), alpha = 0.5) + ## add original data from existing dataframe to new graph in ggplot
  facet_wrap(~Status)
